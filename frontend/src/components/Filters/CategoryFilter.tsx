  ...
  return (
    <div className="categories flex gap-4 items-center">
      {categories.map((category) => {
        if (!category) {
          return <p onClick={()=>{setCategoryFilter(category)}} className={`font-bold text-xl cursor-pointer ${categoryFilter === category && 'active'}`}>
            All
          </p>
        }
        return (

          <div onClick={()=>{setCategoryFilter(category)}} className="h-10">
